...
public class Project2Main {
	...
	public static String m1 = "logistic";
	public static String m2 = "stochastic";
	...
	public static void iterateLogisticRegressorStochasticGradient(
		...
		Set<Integer> ks = testingData.keySet();
		for(Integer k: ks) {
			testingData.get(k).add(0, 1.0);
		}
		...
		//ROCSTAT_ERRORRATELOOGISTIC(WEIGHTVECTORLOGISTICSTOCHASTICREG, testingData);
	}
	...
	public static void iterateLogisticRegressorBatchGradient(
		...
		//ROCSTAT_ERRORRATELOOGISTIC(WEIGHTVECTORLOGISTICBATCHREG, testingData);
	}
	...
	public static void printERRORRATELOGISTIC(ArrayList<Double> w,
		...
		for(Integer key: keysTest) {
			...
			if((dp<=0.45 && row.get(58)==0.0) ||
					(dp>=0.45 && row.get(58)==1.0)) {
				CORRECT++;
			}  else {
				WRONG++;
			}
	...
	public static void iterateLinearRegressorBatchGradient(
		...
		//ROCSTAT_ERRORRATELINEAR(WEIGHTVECTORLINEARBATCHREG, testingData);
	}
	...
	public static String printERRORRATELINEAR(ArrayList<Double> w,
		...
		for(Integer key: keysTest) {
			...
			if((dp<=0.421 && row.get(58)==0.0) ||
					(dp>=0.421 && row.get(58)==1.0)) {
				CORRECT++;
			}  else {
				System.out.println(dp+" : "+row.get(58));
				WRONG++;
	...
	public static String ROCSTAT_ERRORRATELINEAR(ArrayList<Double> w,
			HashMap<Integer, ArrayList<Double>> testingData) {
		Set<Integer> keysTest = testingData.keySet();
		int CORRECT = 0;
		int WRONG = 0;
		...
		int NUMBER_SPAM = 0;
		int NUMBER_NON_SPAM = 0;
		HashMap<Double, ArrayList<Integer>> mapS =
				new HashMap<Double, ArrayList<Integer>>();
		...
		for(Integer key: keysTest) {
			...
			if(mapS.containsKey(dp)) {
				ArrayList<Integer> keysNow = mapS.get(dp);
				keysNow.add(key);
				mapS.put(dp, keysNow);
			} else {
				ArrayList<Integer> keysNow = new ArrayList<Integer>();
				keysNow.add(key);
				mapS.put(dp, keysNow);
			}
		}
		...
		for(int i=dps.size()-1; i >=0 ; i--) {
			double d = dps.get(i);
			ArrayList<Integer> keys = mapS.get(d);

			for(Integer key: keys) {

				if(testingData.get(key).get(58)==1.0){
					TRUEPOSITIVE++;
				} else {
					FALSEPOSITIVE++;
				}

				/*if((d<=0.421 && testingData.get(key).get(58)==0.0) ||
						(d>=0.421 && testingData.get(key).get(58)==1.0)) {
					CORRECT++;
					...
					if((d<=0.421 && testingData.get(key).get(58)==1.0)) {
						FALSEPOSITIVE++;
					} else {
						FALSENEGATIVE++;
					}*/
				double TPR = (double)TRUEPOSITIVE/(double)(NUMBER_SPAM);
				double FPR = (double)FALSEPOSITIVE/(double)(NUMBER_NON_SPAM);
	...
	public static String ROCSTAT_ERRORRATELOOGISTIC(ArrayList<Double> w,
			HashMap<Integer, ArrayList<Double>> testingData) {
		Set<Integer> keysTest = testingData.keySet();
		int CORRECT = 0;
		int WRONG = 0;
	...
	public static void iterateLinearRegressorStochasticGradient(
		...
		//ROCSTAT_ERRORRATELINEAR(WEIGHTVECTORLINEARSTOCHASTICREG, testingData);
	}
	...
	public static HashMap<Integer, ArrayList<Double>> getZScoredMap(){
		...
		ArrayList<Double> means = ProjUtils.returnAllMeansAsList(map);
		ArrayList<Double> sds = ProjUtils.getAllStandardDeviations(map, means, 57);		
		return ProjUtils.returnZScoredMap(map, means, sds);
	}
